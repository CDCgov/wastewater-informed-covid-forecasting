library(argparser)
pull_and_write <- function(output_dir,
                 force = FALSE) {
    ...
    data <- raw_data |>
        dplyr::mutate(
                   daily_hosp_admits =
                       as.numeric(.data$previous_day_admission_adult_covid_confirmed) +
                       as.numeric(.data$previous_day_admission_pediatric_covid_confirmed),
                   date = as.Date(.data$date) - lubridate::ddays(1)) |>
                                        # convert from previous day to date-of-event indexing, following covidcast
        dplyr::select(
                   date, ABBR=state, daily_hosp_admits)
...
argv <- parse_args(p)
pull_and_write(output_dir = argv$output_dir,
               force = argv$force)
