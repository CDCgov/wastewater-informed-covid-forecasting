...
ggplot(cv) +
  aes(x = value) +
  stat_halfeye() +
  xlab("Coefficient of variation in individual dispersion in number of genomes shed")
mu_g <- all_draws %>%
  spread_draws(mu_g) %>%
  mutate(draw = row_number()) %>%
  mutate(
...
ggplot(mu_g) +
  aes(x = value) +
  stat_halfeye() +
  xlab("Mean number of genomes shed per infection")

log_genomes_shed_per_inf <- all_draws %>%
  spread_draws(log_genomes_shed_per_inf) %>%
  mutate(draw = row_number()) %>%
  mutate(
    name = "log_genomes_shed_per_inf",
  ) %>%
  rename(value = log_genomes_shed_per_inf) %>%
  select(name, value, draw)
ggplot(log_genomes_shed_per_inf) +
  aes(x = value) +
  stat_halfeye() +
  xlab("Log of individual's number of genomes shed per infection ")

genomes_shed_per_inf <- all_draws %>%
  spread_draws(log_genomes_shed_per_inf) %>%
  mutate(draw = row_number()) %>%
  mutate(
    name = "genomes_shed_per_inf",
  ) %>%
  mutate(value = exp(log_genomes_shed_per_inf)) %>%
  select(name, value, draw)
ggplot(genomes_shed_per_inf) +
  aes(x = value) +
  stat_halfeye() +
  xlab("Individual's number of genomes shed per infection ")
```
Hospital admissions
```{r}
...
ww_site_sigma_draws <- all_draws %>%
  ...
  left_join(site_map)
sigma_ww_site_mean <- all_draws %>%
  spread_draws(sigma_ww_site_mean) %>%
  rename(value = sigma_ww_site_mean) %>%
  mutate(
    draw = row_number(),
    name = "mean_sigma_ww_site"
  )
...
ggplot(sigma_ww_site_mean) +
  aes(x = value) +
  stat_halfeye() +
  xlab("Mean scale parameter of lognormal obs process ")
...
ggplot(data = ww_site_sigma_draws, aes(y = as.factor(ww_pop), x = value)) +
  ...
  xlab("site-level observation error ") +
