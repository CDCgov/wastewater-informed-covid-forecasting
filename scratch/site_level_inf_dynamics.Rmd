...
```
```{r}
stanfit <- rstan::read_stan_csv(fit_dynamic_rt$output_files())
posterior_cp <- as.array(stanfit)
available_mcmc(pattern = "_nuts_")
np_cp <- nuts_params(stanfit)
mcmc_pairs(posterior_cp,
  np = np_cp, pars = c("log10_g", "cv", "sigma_ww_site_mean", "sigma_ww_site_sd"),
...
log_total_g <- all_draws %>%
  spread_draws(log_total_g[site_index, t]) %>%
  rename(value = log_total_g) %>%
  mutate(
    draw = `.draw`,
    name = "log_total_g"
  ) %>%
