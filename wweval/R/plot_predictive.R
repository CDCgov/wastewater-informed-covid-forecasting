...
plot_ribbon_hosp_quantiles <- function(
  ...
  if (!("model" %in% colnames(hosp_quantiles))) {
    hosp_quantiles$model <- hosp_quantiles$model_type
  }

  quantiles_wide <- hosp |>
    dplyr::filter(.data$quantile %in% c(0.025, 0.25, 0.5, 0.75, 0.975)) |>
    tidyr::pivot_wider(
      id_cols = tidyselect::any_of(c(
        "location",
        "forecast_date",
        "period",
        "scenario",
...
plot_ribbon_ww_quantiles <- function(
  ...
  p <- ggplot(quantiles_wide) +
    ...
    facet_grid(.data$location ~ .data$site_lab_name, scales = "free_y") +
    xlab("") +
    ylab("Genome copies per mL") +
    scale_x_date(
      date_breaks = "2 weeks",
