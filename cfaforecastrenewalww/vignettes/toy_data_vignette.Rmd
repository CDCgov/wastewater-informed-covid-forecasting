...
The model expects input data in a certain format, we will use a built in function
that requires a few other pieces of information that can be derived directly
from the input data set. We pre-specify the priors in the `get_params()`
function. Priors can be adjusted by directly modifying the value in the `params`
dataframe or by directly modifying the `stan_data` list (or alternatively, by
directly editing the stan file but this isn't recommended as the current
workflow keeps parameters consistent across the related models).
```{r}
params <- get_params()
...
# Predicted observed wastewaster concentrations in each lab site
...
Get a dataframe of all of the key estimate model paramters combined with the key
