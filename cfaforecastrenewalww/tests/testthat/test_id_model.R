test_that("Test the site-level infection dynamics model on simulated data.", {
  #######
  ...
  exp_par_names <- get_nonmatrix_names_from_draws(toy_stan_fit_last_draw_id)
  ...
  exp_par_lens <- get_par_dims_flat(toy_stan_fit_last_draw_id)
  ...
  for (param in model_params) {
    expect_equal(
      ...
      posterior::subset_draws(toy_stan_fit_last_draw_id, variable = !!param)
    )
  }
