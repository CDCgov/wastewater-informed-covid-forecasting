...
transformed parameters {
 ...
 // row_vector [ot + uot + ht] model_net_i; // number of net infected individuals shedding on each day (sum of individuals in dift stages of infection)
  ...
  for (i in 1:n_subpops) {
    ...
    total_g[i] = to_row_vector(convolve_dot_product(to_vector(exp(zeta_bar[i])),
                                reverse(s), (uot + ot + ht)));
    log_total_g[i] = log(total_g[i]);
    //print("Log(total_g): ", log10_g*log(10)+ log(total_g[i, uot +2]));
    //log_total_g[i] = to_row_vector(
    //  log_convolve(
    //     reverse(log(s)),
    //     log10_g*log(10) + to_vector(zeta_bar[i])
    //  )
    //);
...
model {
  ...
  sigma_ww_site_mean ~ normal(0.1*sigma_ww_site_prior_mean_mean, 0.5*sigma_ww_site_prior_mean_sd);
  sigma_ww_site_sd ~ normal(sigma_ww_site_prior_sd_mean, 0.1*sigma_ww_site_prior_sd_sd);
    ...
    if (include_ww == 1) {
      ...
      for (i in 1:n_subpops){
          for (j in 1:(uot + ot + ht)){
            zeta_bar[i,j] ~ expgamma(i_site_t[i,j]/(cv^2), cv^2); // Sum of iid gamma individual relative shedding intensities
          }
